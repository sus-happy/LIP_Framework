<?php
/* -----------------------------
 LIP_Database_Common : データベース共通クラス
 /app/LIP/include/database/common.php
 --
 @written 12-12-18 SUSH
----------------------------- */

class LIP_Database_Common extends LIP_Object {
    private $db    = NULL,
            $sql   = array(),
            $args  = array(),
            $save  = FALSE,
    ...
    public function add_order( $target, $duration = "ASC" ) {
        ...
        public function add_order_field( $target, $field ) {
            ...
            foreach( $field as $string ) {
                $query[] = sprintf( 'WHEN ? THEN %d', $count );
                $this->args['ORDER'][] = $string;
                $count ++;
            }

            $this->sql['ORDER'][] = sprintf( 'CASE `%s` %s END', $target, implode( ' ', $query ) );
            return TRUE;
        }
    ...
    public function set_unsave_group( $groups ) {
        if(! is_array( $groups ) ) {
            $this->push_error( 'OPTION', 'Unsave Groups is should be an Array' );
        } else
            $this->unsave_group = $groups;
    }
}